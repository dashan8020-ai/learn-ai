...
> 架构详解（注意力机制、位置编码、FFN 设计、Mamba/SSM 原理等）见 [序列建模架构](./sequence-architectures.md)。
...
> 注意力变体（MHA/GQA/MLA）、位置编码（RoPE/ALiBi）、归一化、FFN/激活函数、MoE 等架构层面的技术已移至 [序列建模架构](./sequence-architectures.md)。本节聚焦 LLM 特有的技术维度。
...
长上下文的关键技术：
- **RoPE 频率外推**: 将 RoPE 基础频率调大（详见 [序列建模架构 § 1.5](./sequence-architectures.md#15-位置编码的演进)）
- **持续预训练**: 在长文本上继续训练
- **Flash Attention**: 降低内存占用（详见 [序列建模架构 § 1.4](./sequence-architectures.md#14-flash-attention-io-感知的算法革新)）
